...
@section('admin.content')
    <div id="layoutSidenav_content">
        <main>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <form action="{{ route('admin.products.update',$products->id)  }}" method="POST" role="form">
                        @csrf
                        @method('POST')
                        <div class="form-group">
                            <label for="name" class="inline">商品名稱：</label>
                            <input name="name" class="form-control-itemname" placeholder="請輸入商品名稱" value="{{ old('name',$products->name) }}">
                        </div>

                        <div class="form-group">
                            <label for="quan" class="inline">庫存：</label>
                            <input name="quan" type="number" class="form-control-itemname" placeholder="請輸入目前庫存數量" value="{{ old('quan',$products->quan) }}">
                        </div>
